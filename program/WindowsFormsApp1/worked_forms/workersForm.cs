...
namespace WindowsFormsApp1
{
    public partial class workersForm : Form
    {
        ...
        private void workersForm_Load(object sender, EventArgs e)
        {
            ...
            db = new DateBase();
            db.openConection();
            adapted = new MySqlDataAdapter("SELECT * FROM `специалисты`", db.getConnection());
            table = new DataTable();
        ...
        public Boolean if_data_exists_date()
        {
            ...
            command_if = new MySqlCommand("SELECT * FROM `специалисты` WHERE `ФИО` = @FIO", db.getConnection());
            ...
            {
                MessageBox.Show("Данный специалист с такими же ФИО уже существует");
                return true;
            }
            else
            {
        ...
        public Boolean if_data_exists_users()
        {
            ...
            command_if.Parameters.Add("@login1", MySqlDbType.VarChar).Value = dataGridView1.Rows[index].Cells[3].Value;
        ...
        private void добавитьНовыеДанныеToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            command_insert_user.Parameters.Add("@l1", MySqlDbType.VarChar).Value = dataGridView1.Rows[index].Cells[3].Value;
            command_insert_user.Parameters.Add("@l2", MySqlDbType.VarChar).Value = dataGridView1.Rows[index].Cells[4].Value;


            command_insert = new MySqlCommand("INSERT INTO `специалисты` (`ФИО`, `id_специализации`, `Логин`, `Пароль`) VALUES (@ff1, @ff2, @ff3, @ff4)", db.getConnection());
        ...
        private void изменитьДанныеToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            command_update = new MySqlCommand("UPDATE `специалисты` SET `ФИО` = @f1, `id_специализации` = @f2 WHERE `специалисты`.`ID_специалиста` = @f0", db.getConnection());
            ...
            try
            {
                if (command_update.ExecuteNonQuery() == 1)
                    MessageBox.Show("Запись специалиста была обновлена");
                else
        ...
        private void удалитьДанныеToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            command_delete = new MySqlCommand("DELETE FROM `специалисты` WHERE `специалисты`.`ID_специалиста` = @f0", db.getConnection());
            ...
            command_delete_user.Parameters.Add("@ff0", MySqlDbType.VarChar).Value = dataGridView1.Rows[index].Cells[3].Value;

            try
            {
                if (command_delete.ExecuteNonQuery() == 1)
                    MessageBox.Show("Аккаунт специалиста был удалён");
                else
                    MessageBox.Show("Oшибка удаления аккаунта");

                if (command_delete_user.ExecuteNonQuery() == 1)
                    MessageBox.Show("Запись специалиста была удалена");
                else
